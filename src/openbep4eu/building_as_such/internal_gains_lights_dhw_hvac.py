...
def get_equipment_power_density(config: EquipmentConfig, floor_area: Optional[float] = None) -> float:
    ...
    if floor_area <= 0:
        raise ValueError("Floor area must be greater than zero for equipment calculations.")
...
def calculate_single_lighting_appliance_heat_gain(
    ...
    if equipment.fuelType not in included_fuel_types:
